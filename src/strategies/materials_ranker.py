"""
Materials Relative Strength Ranker - Position Trading
======================================================
Scans for relative strength leaders in Materials sector.

Entry Criteria:
- Sector: Materials (XLB > MA200, rising)
- Relative Strength: RS >= +18% vs SPY (6-month)
...
Position Sizing: 1.25% risk per trade
Max Positions: 2-3
...
def scan_materials(tickers, as_of_date, sector_etf_df, adx_threshold=25, mat_logged_tickers=None):
    """Scan for Materials RS leaders."""
    signals = []
    
    if mat_logged_tickers is None:
        mat_logged_tickers = set()
    ...
    for ticker in tickers:
        ...
        can_enter = (
            ...
            rs is not None and rs >= 18.0  # RS >= +18%
        )
        ...
        if can_enter:
            ...
            signal = {
                ...
                'Strategy': 'Materials_Ranker_Position',
            ...
            if ticker not in mat_logged_tickers:
                print(f"   ✅ Materials: {ticker} RS={rs:+.1f}% ADX={adx.iloc[-1]:.0f}")
                mat_logged_tickers.add(ticker)
