"""
Energy Relative Strength Ranker - Position Trading
===================================================
Scans for relative strength leaders in Energy sector.

Entry Criteria:
- Sector: Energy (XLE > MA200, rising)
...
def scan_energy(tickers, as_of_date, sector_etf_df, adx_threshold=25, energy_logged_tickers=None):
    """Scan for Energy RS leaders."""
    signals = []
    
    if energy_logged_tickers is None:
        energy_logged_tickers = set()
    ...
    for ticker in tickers:
        ...
        if can_enter:
            ...
            signal = {
                ...
                'Strategy': 'Energy_Ranker_Position',
            ...
            if ticker not in energy_logged_tickers:
                print(f"   ✅ Energy: {ticker} RS={rs:+.1f}% ADX={adx.iloc[-1]:.0f}")
                energy_logged_tickers.add(ticker)
